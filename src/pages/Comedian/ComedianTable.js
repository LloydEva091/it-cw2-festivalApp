...
import { updateComedian, deleteComedian } from '../../apis/festivalApi'

import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';


const ComedianTable = ({ comedianD }) => {
    ...
    const { isError, error: errorMSG } = useMutation(deleteComedian)
    ...
    const updateComedianMutation = useMutation(
        () => updateComedian({
            id: formData.id,
            name: formData.name,
    ...
    const deleteMutation = useMutation(
        () => deleteComedian({ id: formData.id }, user.token),
        {
            onSuccess: () => {
                queryClient.invalidateQueries("comediansCache")
            },
    ...
    const handleSubmit = async () => {
        ...
        try {
            await updateComedianMutation.mutateAsync(({
                id: formData.id,
                name: formData.name,
                description: formData.description,
                image: formData.description
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            console.log(error)
            console.log(errMsg)
            notify("Comedian has an event assigned. Cannot delete");
        }
    }
                                                ...
                                                <div className="flex item-center justify-center">
                                                    <div className="w-4 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                        <div type="button" onClick={() => {
                                                            setShowModal(true)
                                                            setFormData({
                                                                id: items._id, name: items.name, description: items.description, image: items.image
                                                            })
                                                        }}>
                                                                    ...
                                                                    <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                                                                        ...
                                                                        <div className="relative p-6 flex-auto h-full">
                                                                            <form className="flex flex-col " onSubmit={handleSubmit}>
                                                                                ...
                                                                                <label htmlFor="name" className='font-bold text-black'>COMEDIAN NAME</label>
                                                                                <InputControl
                                                                                    label="Comedian name"
                                                                                    type="name"
                                                                                ...
                                                                                <label htmlFor="name" className='font-bold text-black p-2'>IMAGE URL</label>
                                                                                <InputControl
                                                                                    label="Image"
                                                                                    type="image1"
